...
bool Generator::writeIconValues() {
	...
	for (auto i = iconMasks_.cbegin(), e = iconMasks_.cend(); i != e; ++i) {
		...
		if (filePath.startsWith("size://")) {
			...
			const auto svgPath = iconMaskSvgPath(filePath);
		...
		if (!filePath.startsWith("size://") && hasOverridePngScales(filePath)) {
			...
			if (!overrideData.isEmpty()) {
				hasOverrideData.insert(i.value());
	...
	for (auto i = iconMasks_.cbegin(), e = iconMasks_.cend(); i != e; ++i) {
		...
		if (hasOverrideData.contains(i.value())) {
			source_->stream() << "IconMask iconMask" << i.value() << "(iconMask" << dataIndex << "Data, iconMask" << i.value() << "OverrideData" << sizeArgument << ");\n";
		} else {
